...
enqueue(Value, {Front, Back}) -> {Front, [Value|Back]}.

dequeue({[],[]}) -> create();
dequeue({[], One}) -> create();
%dequeue({[One], Back}) -> {lists:reverse(Back), []};
dequeue({[One], Back}) -> 
    {List1, List2} = lists:split(length(Back) div 2, Back),
    {lists:reverse(List2), List1};
dequeue({[First|Rest], Back}) -> {Rest, Back}.
