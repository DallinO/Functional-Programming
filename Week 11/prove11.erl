...
add(Word, nil) -> 
    put(Word, #{});
add([], Node) ->
    case contains(done, Node) of
        true -> put(done, nil, Node);
        false -> put(done, nil, Node)
    end;
add([First|Rest], Node) ->
    case contains(First, Node) of
        true -> put(First, add(Rest, get(First, Node)), Node);
        false -> put(First, add(Rest, #{}), Node)
    end.
